...
export class GroupItem extends React.Component<GroupItemProps> {
    ...
    render() {
        const { item } = this.props;
        return <>
            <Form className="questionnaire-group-item__headline" getApi={this.getFormApi.bind(this)} key={item.id} initialValues={item} onSubmit={this.handleSubmit.bind(this)}>
